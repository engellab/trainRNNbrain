...
class RNN_torch(torch.nn.Module):
    ...
    def forward(self, u, w_noise=True):
        ...
        saved_hs = [torch.zeros(self.N, batch_size, device=self.device)]

        for i in range(1, T_steps):
            h = self.W_rec @ states_list[-1] + self.W_inp @ (u[:, i - 1, :] + inp_noise[:, i - 1, :])
            saved_hs.append(h)  # Save original h, which contributes to states
            next_state = states_list[-1] + self.alpha * (-states_list[-1] + self.activation(h) + rec_noise[:, i - 1, :])
            states_list.append(next_state)

        # Now return this list of computation-linked h values:
        hs_new = torch.stack(saved_hs, dim=1)  # shape: (N, T-1, B)
        states_new = torch.stack(states_list, dim=1)  # shape (batch, T_steps, N)
        outputs = torch.einsum("oj,jtk->otk", self.W_out, self.activation(hs_new))
        return states_new, outputs, hs_new
