...
class RNN_torch(torch.nn.Module):
    ...
    def adversarial_dropout_mask(self, participation, drop_rate, adv_strength=5.0):
        # Normalize to [0, 1]
        p = (participation - participation.min()) / (participation.max() - participation.min() + 1e-8)
        drop_probs = drop_rate * torch.softmax(adv_strength * p, dim=0) * p.shape[0]
        drop_probs = torch.clamp(drop_probs, 0, 1.0)
        mask = torch.bernoulli(1 - drop_probs).unsqueeze(-1)
        mask = mask / (1 - drop_probs.unsqueeze(-1) + 1e-8)
        return mask
    ...
    def forward(self, u, w_noise=True, dropout=False, drop_rate=0.05, participation=None):
        ...
        for t in range(1, T_steps):
            ...
            if dropout:
                s = states_list[-1]
                p = torch.mean(torch.abs(s), dim=-1) + torch.std(s, dim=-1) # across batch
                dropout_mask = self.adversarial_dropout_mask(participation=p, drop_rate=drop_rate)
