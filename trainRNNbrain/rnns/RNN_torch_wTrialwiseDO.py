...
class RNN_torch(torch.nn.Module):
    ...
    def forward(self, u, w_noise=True, dropout=False, drop_rate=0.3):
        ...
        # Structural dropout: sample mask ONCE per trial (batch)
        if dropout:
            ...
            dropout_mask = torch.ones((self.N, batch_size), device=self.device)

        states_list = [states[:, 0, :] * dropout_mask]  # ensure correct at t=0

        for t in range(1, T_steps):
            s_masked = states_list[-1] * dropout_mask  # zero out deaf-mute neurons
            ...
            next_state = (s_masked + self.alpha * rhs_val) * dropout_mask  # keep deaf-mute at zero
