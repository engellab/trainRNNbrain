...
class Trainer():
    ...
    #                      low_percent=0.1, high_percent=0.6,
    ...
    def net_inputs_penalty(self, states, input,
                           ...
                           beta=20.0, alpha=0.2, k=20.0, eps=1e-6):
        """
        Bounded, scale-stable penalty:
          - One helper that takes a positive "violation" g (>=0 means violating)
          - Softplus for smooth one-sided barrier
          - LogSumExp normalization (smooth max) for scale robustness
        ...
        def side_penalty(g):
            # Smooth one-sided violation (≈ReLU for g>0)
            v = F.softplus(g, beta=beta)  # >= 0, big when g>0
            # Soft mask: ~1 if violating, ~0 otherwise
            m = torch.sigmoid(k * g).detach()
            # Normalize by smooth max to make dimensionless & stable
            lse = (alpha * torch.logsumexp(v / alpha, dim=0)).detach() + eps
            v_norm = v / lse
            # Mean over violators only (softly)
            pen = (v_norm * m).sum() / (m.sum() + eps)
            # Bounded squasher in [0,1)
            return (pen) / (1.0 + pen)
        ...
        low_penalty = side_penalty(low_gap)
        high_penalty = side_penalty(high_gap)
    ...
    def train_step(self, input, target_output, mask):
        ...
        net_inputs_variability_penalty = self.lambda_hvar * self.net_inputs_variability_penalty(states, input) if self.lambda_hvar != 0 else 0
        isolation_penalty = self.lambda_iz * self.isolation_penalty() if self.lambda_iz != 0 else 0
        categorical_penalty = self.lambda_cat * self.categorical_penalty(states) if self.lambda_cat != 0 else 0
