...
class Trainer():
    ...
    def categorical_penalty(self, states, beta=20.0, ratio_target=0.5):
        X = states.view(states.shape[0], -1)
        loss = torch.tensor(0.0, device=states.device, dtype=states.dtype)
        if self.RNN.dale_mask is None:
        ...
        for nrn_sign in [1, -1]:
            ...
            D_masked = D[mask].view(D.shape[0], -1)
            nearest = D_masked.min(dim=1)[0]  # [N_sub] nearest neighbor distance for each neuron
            median_pairwise = D_masked.median()

            ratio = nearest.mean() / (median_pairwise + 1e-8)
            penalty = F.softplus(ratio - ratio_target, beta=beta)
            loss += penalty
        return loss
