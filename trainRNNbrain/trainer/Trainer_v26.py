...
class Trainer():
    ...
    def gini_penalty(
            ...
            max_exp: float = 8.0,  # clamp exponent for numeric stability
            min_iqr: float = 1e-3  # floor to avoid division by ~0
    ):
        v = x.reshape(-1)
        if v.numel() <= 1:
            return torch.zeros((), dtype=v.dtype, device=v.device)
        # Robust scale
        q_lo, q_hi = torch.quantile(v, p_lo), torch.quantile(v, p_hi)
        iqr = torch.clamp(q_hi - q_lo, min=min_iqr)
        center = v.mean()

        if detach_stats:
            iqr = iqr.detach()
            center = center.detach()
        z = torch.abs(v - center) / (iqr + eps)
        if max_exp is not None: z = torch.clamp(z, -max_exp, max_exp)
        u = torch.exp(z)
    ...
    def activity_penalty(self, states):
        ...
        return self.gini_penalty(activity) + 0.1 * torch.mean(torch.abs(activity)) + torch.std(activity)
    ...
    def net_inputs_penalty(self, states, inp):
        ...
        return self.gini_penalty(mean_h) + torch.std(mean_h)
