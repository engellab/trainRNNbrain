...
def qn_scale(x: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    v = x.reshape(-1)
    n = v.numel()
    # Pairwise absolute differences (upper triangle only)
    diffs = torch.abs(v[:, None] - v[None, :])     # (n,n)
    iu = torch.triu_indices(n, n, offset=1, device=v.device)
    d = diffs[iu[0], iu[1]]                        # (n*(n-1)/2,)
    # First quartile of pairwise distances
    q = torch.quantile(d, 0.25)
    # Gaussian consistency constant ≈ 2.2219
    scale = 2.2219 * q
...
class Trainer():
    ...
    def gini_penalty(
        ...
        scale_floor = (1.4826 * mad).clamp_min(1e-6)
        scale_qn = qn_scale(v)
        scale = torch.sqrt(scale_qn*scale_qn + scale_floor*scale_floor)

        if detach_stats:
            scale = scale.detach()
            center = center.detach()
        z = torch.abs(v - center) / (scale + eps)
        if max_z is not None: z = torch.clamp(z, -max_z, max_z)
        u = torch.sqrt(1.0 + z*z)
    ...
    def net_inputs_variability_penalty(self, states, inp):
        # Net inputs
        h = (torch.einsum('ij,jkl->ikl', self.RNN.W_rec, states) +
             torch.einsum('ij,jkl->ikl', self.RNN.W_inp, inp))  # (N, T, B)
        net_input_var_t = torch.mean(torch.std(h, dim=1, unbiased=False), dim = -1)
        return torch.mean(net_input_var_t) #minimizing variability acorss time!!
