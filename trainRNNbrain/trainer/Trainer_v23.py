...
class Trainer():
    ...
    @staticmethod
    def gini_penalty(x, eps=1e-8, tau=0.0):
        """
        Gini inequality (no sort). Larger => more unequal. Use for nonnegative stats.
        x: any shape -> flattened. If you may have small negatives from noise, clamp before call.
        """
        v = x.reshape(-1)
        if v.numel() <= 1:
    ...
    @staticmethod
    def tail_penalty(x, tail='both', eps=1e-6, zcap=3.0):
        """
        Scale-invariant tail heaviness on z-scored values via layer_norm.
        Larger => heavier tails. Use for signed stats (e.g., net inputs).
        """
        v = x.reshape(-1)
        if v.numel() <= 1:
            return torch.zeros((), dtype=v.dtype, device=v.device)
        z = F.layer_norm(v.float(), v.shape, None, None, eps).to(v.dtype)
        if tail == 'right':
            u = torch.clamp(z, min=0.0, max=float(zcap))
        elif tail == 'left':
            u = torch.clamp(-z, min=0.0, max=float(zcap))
        elif tail == 'both':
            u = torch.clamp(torch.abs(z), max=float(zcap))
        else:
            raise ValueError("tail must be 'right','left','both'")
        return torch.mean(u ** 4)
    ...
    def activity_penalty(self, states, eps=1e-6):
        ...
        # return self.tail_penalty(activity, eps=eps, tail='right')
    ...
    def net_inputs_penalty(self, states, inp, eps=1e-6):
        ...
        return self.tail_penalty(mean_h, eps=eps, tail='both')
    ...
    def net_inputs_variability_penalty(self, states, inp, eps=1e-8):
        ...
        return self.gini_penalty(neural_trial_variability)
        # # Low-tail penalty
        # return self.tail_penalty(neural_trial_variability, eps=eps, tail='left')
