...
class Trainer():
    ...
    def inactivity_penalty(self, states):
        participation = torch.std(states, dim=(1, 2))  # shape: (N,)
        max_participation = torch.max(participation).detach() + 1e-8
        normalized_participation = participation / max_participation

        threshold = 0.3
        low_participation = normalized_participation[normalized_participation < threshold]

        if low_participation.numel() == 0:
            return torch.tensor(0.0, device=states.device)  # no penalty if all neurons are active

        penalty = 1 - torch.nanmean(low_participation) / threshold
        return penalty
