...
class Trainer():
    ...
    def dimensionality_penalty(self, states, gini_target=0.8, beta=20.0, eps=1e-8):
        """
        Encourages moderately low-dimensional representations by targeting the Gini index
        of the singular values (computed on [N, T*K] reshaped states).
        """
        N, T, K = states.shape
        ...
        S_sorted = S_norm.flip(dims=[0])
        # Gini index formula
        n = S_sorted.numel()
        index = torch.arange(1, n + 1, device=states.device, dtype=states.dtype)
        gini = (2 * (index * S_sorted).sum() / (n * S_sorted.sum() + eps)) - (n + 1) / n
        # Penalize if Gini is lower than target
        penalty = F.softplus(gini_target - gini, beta=beta)
        return penalty


    def categorical_penalty(self, states, dale_mask=None, beta=20.0, q_low=0.1, q_high=0.7):
        X = states.view(states.shape[0], -1)  # [N, D]
        ...
        for nrn_sign in [1, -1]:
            X_subpop = X[dale_mask == nrn_sign, :]  # [N_sub, D]
            if X_subpop.shape[0] <= 1:
                continue
            # Compute centroid and per-neuron distances to center
            center = torch.mean(X_subpop, dim=0, keepdim=True)
            d_to_center = torch.norm(X_subpop - center, dim=1)  # [N_sub]
            # Quantile thresholds for the cloud shape
            r_low = torch.quantile(d_to_center, q=q_low).detach()
            r_high = torch.quantile(d_to_center, q=q_high).detach()
            eps = 1e-8
            ...
            # Extract upper triangle (no self or duplicate pairs)
            i, j = torch.triu_indices(D.shape[0], D.shape[1], offset=1)
            dists = D[i, j]
            # Attraction: for pairs closer than r_low
            attract = F.softplus((r_low - dists) / (r_low + eps), beta=beta)
            # Repulsion: for pairs farther than r_high
            repulse = F.softplus((dists - r_high) / (r_high + eps), beta=beta)
            loss += torch.mean(attract + repulse)
        return loss
