...
class Trainer():
    ...
    def activity_penalty(self, states, high_percent=0.7, hard_threshold=0.5, gamma=8.0, beta=20.0, eps=1e-6):
        '''
        Penalizes excessive activity of units
        '''
        # Per-neuron mean absolute activity
        activity = torch.mean(states, dim=(1, 2))  # (N,)
        hard_threshold = torch.tensor(hard_threshold, device=self.RNN.device)
        dyn_threshold = torch.quantile(activity, q=high_percent).detach()
        threshold = torch.min(dyn_threshold, hard_threshold)
