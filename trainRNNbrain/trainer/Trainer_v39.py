...
class Trainer():
    def __init__(self,
        ...
        self.penalty_map = {
            ...
            "s_magnitude": (self.s_magnitude_penalty, lambda_sm, {"cap_low": cap_low, "cap_high": cap_high, "q":0.9, "alpha":5.0, "beta":1.0}),
    ...
    def s_magnitude_penalty(self, states, input, cap_low=0.05, cap_high=0.7, q=0.9, p=3.0, alpha=5.0, beta=1.0, eps=1e-12):
        ...
        e_lo = torch.log2((s_q + eps) / (clo + eps))  # <0 if below lower cap
        e_hi = torch.log2((s_q + eps) / (chi + eps))  # >0 if above upper cap

        under = torch.pow(2.0, p * torch.relu(-e_lo)) - 1.0  # strong push from zero
        over = torch.pow(2.0, alpha * torch.relu(e_hi)) - 1.0  # steep over-cap
        return (under + beta * over).mean()
    ...
    def enforce_io_nonnegativity(self):
        with torch.no_grad():
            self.RNN.W_inp.clamp_min_(0)
            self.RNN.W_out.clamp_min_(0)
        return None
