...
class Trainer():
    def __init__(self,
        ...
        self.penalty_map = {
            ...
            "metabolic": (self.metabolic_penalty, self.lambda_met),
    ...
    def inp_weights_magnitude_penalty(self, states, input, cap100=5.0):
        # scales like O(log(N)/N)
        scale = (torch.log1p(torch.as_tensor(self.RNN.N, device=self.RNN.device, dtype=states.dtype)) /
                 torch.log1p(torch.as_tensor(100, device=self.RNN.device, dtype=states.dtype))) / self.RNN.N
        cap = cap100 * scale
        return torch.expm1(torch.relu(self.RNN.W_inp.abs() - cap) / cap).mean()

    def out_weights_penalty(self, states, input, c=2.0, cap100=5.0, gamma=1.0, eps=1e-12):
        # scales like O(log(N)/N)
        scale = (torch.log1p(torch.as_tensor(self.RNN.N, device=self.RNN.device, dtype=states.dtype)) /
                torch.log1p(torch.as_tensor(100, device=self.RNN.device, dtype=states.dtype))) / self.RNN.N
    ...
    def rec_weights_magnitude_penalty(self, states, input, cap100=0.07, N_ref=100, k_ref=20):
        W = self.RNN.W_rec
        exc, inh = self.RNN.dale_mask > 0, self.RNN.dale_mask < 0
        scale = torch.log1p(torch.as_tensor(100.0, device=self.RNN.device, dtype=states.dtype)) / \
                torch.log1p(torch.as_tensor(self.RNN.N, device=self.RNN.device, dtype=states.dtype))
        cap = cap100 * scale # scales as O(1 / log(N))
    ...
    def s_magnitude_penalty(self, states, input, cap100=0.2, q=0.9, alpha=5.0, beta=1.0, eps=1e-12):
        x = states.abs().view(states.size(0), -1)  # (N, T*B)
        dev, dt = states.device, states.dtype
        cap100 = torch.as_tensor(cap100, device=dev, dtype=dt)
        scale = torch.log1p(torch.as_tensor(100.0, device=dev, dtype=dt)) / \
        ...
        e = torch.log((s_q + eps) / (cap + eps))  # >0 over, <0 under
        return (torch.relu(-e).pow(2) + beta * torch.expm1(alpha * torch.relu(e))).mean()
    ...
    def train_step(self, input, target_output, mask):
        ...
        for k, (fn, lam) in self.penalty_map.items():
            if lam == 0: continue
        ...
        with torch.no_grad():
            loss_log = behavior_mismatch_penalty + sum(
                (self.penalty_map[k][1] * penalty_dict_raw[k]) for k in self.penalty_map if self.penalty_map[k][1] != 0
)
