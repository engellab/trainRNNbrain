...
class Trainer():
    ...
    def inactivity_penalty(self, states, frac=0.3, spread_floor=1e-8):
        participation = torch.mean(torch.std(states, dim=(1, 2)), dim=(1, 2))
        min_p = torch.min(participation).detach()
        max_p = torch.max(participation).detach()
        spread = torch.clamp(max_p - min_p, min=spread_floor)
        threshold = min_p + frac * spread
