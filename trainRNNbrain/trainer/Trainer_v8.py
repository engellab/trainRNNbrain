...
class Trainer():
    ...
    def isolation_penalty(self, percent=0.2):
        # Compute per-neuron mean absolute incoming weights
        incoming_rec = torch.mean(torch.abs(self.RNN.W_rec), dim=1)
        incoming_inp = torch.mean(torch.abs(self.RNN.W_inp), dim=1)
        # Use the means as dynamic thresholds
        mean_rec = torch.quantile(incoming_rec, percent).detach()  # Detach to avoid affecting gradients
        mean_inp = torch.quantile(incoming_inp, percent).detach()
        # Penalize only neurons below the mean
        penalty_rec = torch.mean((torch.relu(mean_rec - incoming_rec) / mean_rec) ** 2)
        penalty_inp = torch.mean((torch.relu(mean_inp - incoming_inp) / mean_inp) ** 2)
        return 0.5 * (penalty_rec + penalty_inp)
