...
class Trainer():
    ...
    @staticmethod
    def tail_penalty(values, tail='right', vcap=10.0):
        v = values.reshape(-1)
        v = v - torch.mean(v)
        if v.numel() <= 1:
            return torch.zeros((), dtype=v.dtype, device=v.device)
        if tail == 'right':
            u = torch.clamp(v, min=0.0, max=float(vcap))
        elif tail == 'left':
            u = torch.clamp(-v, min=0.0, max=float(vcap))
        elif tail == 'both':
            u = torch.clamp(v.abs(), max=float(vcap))
        else:
            raise ValueError("tail must be one of {'right','left','both'}")
        return torch.mean(u ** 2) + torch.mean(u ** 4) # both variance and kurtosis
    
    # @staticmethod
    # def tail_penalty(values, tail='right', eps=1e-6, zcap=10.0):
    #     """
    #     tail penalty on z-scored values via layer_norm.
    #     Args:
    #         values: Tensor of any shape.
    #         tail: 'right' | 'left' | 'both' — which tail(s) to penalize.
    #         eps: Numerical epsilon for layer_norm.
    #         zcap: Cap on |z| to bound gradients.
    #     Returns:
    #         Scalar mean of clamp(tail(z), 0, zcap)^4.
    #     """
    #     v = values.reshape(-1)
    #     if v.numel() <= 1:
    #         return torch.zeros((), dtype=v.dtype, device=v.device)
    #     z = F.layer_norm(v.float(), normalized_shape=v.shape, weight=None, bias=None, eps=eps)
    #     z = z.to(v.dtype)
    #     if tail == 'right':
    #         u = torch.clamp(z, min=0.0, max=float(zcap))
    #     elif tail == 'left':
    #         u = torch.clamp(-z, min=0.0, max=float(zcap))
    #     elif tail == 'both':
    #         u = torch.clamp(z.abs(), max=float(zcap))
    #     else:
    #         raise ValueError("tail must be one of {'right','left','both'}")
    #     return torch.mean(u ** 2) + torch.mean(u ** 4)
    ...
    def activity_penalty(self, states, eps=1e-6):
        """
        Penalizes excessive activity of units (per-neuron mean |activity|)
        states: (N, T, B)
        """
        activity = torch.mean(torch.abs(states), dim=(1, 2))  # (N,)
        return self.tail_penalty(activity, tail='right')

    def net_inputs_penalty(self, states, inp, eps=1e-6):
        """
        states: (N, T, B)
        inp:    (M, T, B)
        W_rec:  (N, N)
        ...
        return self.tail_penalty(mean_h, tail='both')

    def net_inputs_variability_penalty(self, states, inp, eps=1e-8):
        """
        ...
        # Low-tail penalty
        return self.tail_penalty(neural_trial_variability, tail='left')
