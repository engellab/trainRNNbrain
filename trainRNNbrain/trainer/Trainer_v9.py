...
class Trainer():
    ...
    def inhibition_penalty(self, percent=0.9):
        dale_mask = self.RNN.dale_mask  # shape: [N]
        N = dale_mask.shape[0]
        # Compute 'inhibitedness' per neuron (sum of negative incoming weights)
        inhibitedness_rec = torch.sum(torch.clamp(self.RNN.W_rec, max=0), dim=1)  # [N]
        # Inhibitedness is negative (or zero)
        # Boolean masks for each neuron type
        excitatory_mask = (dale_mask == 1)
        inhibitory_mask = (dale_mask == -1)
        # Quantile thresholds per group, detach to avoid backward flow
        # For inhibition, quantiles are less negative as you go up, so "higher inhibition" is more negative
        # Penalize neurons more inhibited than the group threshold (i.e., more negative)
        exc_threshold_rec = torch.quantile(inhibitedness_rec[excitatory_mask], percent).detach()
        inh_threshold_rec = torch.quantile(inhibitedness_rec[inhibitory_mask], percent).detach()
        # Penalize excess inhibition (values more negative than the threshold)
        penalty_exc_rec = torch.mean(torch.relu((exc_threshold_rec - inhibitedness_rec[excitatory_mask]) / exc_threshold_rec) ** 2)
        penalty_inh_rec = torch.mean(torch.relu((inh_threshold_rec - inhibitedness_rec[inhibitory_mask]) / inh_threshold_rec) ** 2)
        return 0.5 * (penalty_exc_rec + penalty_inh_rec)
    ...
    def activity_penalty(self, states,
        ...
        low_threshold = torch.min(torch.quantile(activity, q=low_percent).detach(), hard_min)
        ...
        low_activity_penalty = torch.mean((torch.relu(low_threshold - activity) / low_threshold) ** 2)
        high_activity_penalty = torch.mean((torch.relu(activity - high_threshold) / high_threshold) ** 2)
    ...
    def train_step(self, input, target_output, mask):
        ...
        inhibition_penalty = self.lambda_z * self.inhibition_penalty() if self.lambda_z != 0 else 0
        ...
        self.loss_monitor["inhibition"].append(to_item(inhibition_penalty))
        return loss.item()
