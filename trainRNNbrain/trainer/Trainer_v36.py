...
class Trainer():
    ...
    def inp_weights_magnitude_penalty(self, states, input, cap=0.3):
        return torch.expm1(torch.relu(self.RNN.W_inp.abs() - cap) / cap).mean()

    def out_weights_penalty(self, states, input, c=2., alpha=10., gamma=1., eps=1e-12):
        W = self.RNN.W_out
        N = self.RNN.N
        A = W.abs()
        # 1) target total gain via row L1
        p_l1 = (A.sum(1) - c).pow(2).mean()
        # 2) per-weight soft cap ~ 1/N
        cap = alpha / N
        # p_cap = torch.relu(A - cap).pow(2).mean()
        p_cap = torch.expm1(torch.relu(A - cap) / cap).mean()
        # 3) anti-concentration via normalized HHI (row-wise)
        P = A / (A.sum(1, keepdim=True) + eps)
        hhi = (P * P).sum(1)  # in [1/n, 1]
        n = A.size(1)
        p_hhi = ((n * hhi - 1.) / (n - 1. + eps)).mean()  # in [0,1]
        return p_l1 + p_cap + gamma * p_hhi

    def rec_weights_magnitude_penalty(self, states, input, cap=0.03, N_ref=100, k_ref=20):
        W = self.RNN.W_rec
    ...
    # def rec_weights_sparsity_penalty(self, states=None, input=None, eps=1e-8):
    #     # square-Hoyer per row: S_i = (||w_i||_1^2) / (||w_i||_2^2), S∈[1, M]; effective support size
    #     l1 = self.RNN.W_rec.abs().sum(dim=1)
    #     l2 = (self.RNN.W_rec.square().sum(dim=1) + eps).sqrt()
    #     S = (l1 * l1) / (l2 * l2)  # (N,)
    #     # normalize to [0,1] using vector length M (size-independence), then squared deviation to target
    #     Sn = (S - 1.0) / (self.RNN.N - 1.0 + eps)  # ∈[0,1]
    #     return torch.mean(Sn)

    def rec_weights_sparsity_penalty(self, k_tgt=20, eps=1e-8):
        W = self.RNN.W_rec
        l1 = W.abs().sum(dim=1)
        l2 = (W.square().sum(dim=1) + eps).sqrt()
        S = (l1 * l1) / (l2 * l2)  # effective support per postsyn row
        over = torch.relu(S - k_tgt)  # only punish when effective degree > k_tgt
        return (over * over).mean() / (k_tgt * k_tgt)

    def s_magnitude_penalty(self, states, input, cap=0.2):
        return (torch.quantile(states.abs(), 0.9) - cap).pow(2).mean()
    ...
    #     B = B / (torch.linalg.vector_norm(B, dim=0, keepdim=True) + eps)
