...
class DynamicSystemAnalyzer():
    ...
    def __init__(self, RNN_numpy, task):
        self.RNN = RNN_numpy
        ...
        self.RNN.run(inputs, sigma_inp=0, sigma_rec=0)
        self.trajectories = np.copy(self.RNN.get_history())
...
class DynamicSystemAnalyzerCDDM(DynamicSystemAnalyzer):
    ...
    def __init__(self, RNN, task):
        DynamicSystemAnalyzer.__init__(self, RNN, task)
