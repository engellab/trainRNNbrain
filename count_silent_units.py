#!/usr/bin/env python3
"""
Estimate the number of silent units per condition for the Silent-ReLU sweep
(CDDM_4a031e). For every trained network we reconstruct the RNN from its saved
LastParams, run the noise-free CDDM validation batch through it (exactly the path
run_experiment.py uses to make participation.png), and score each unit's activity.

A unit is "silent" if it is effectively never active across all task conditions and
time. Two transparent criteria are reported (the story should agree on both):

  - dead_abs   : peak firing rate over (time, conditions) < 0.01
                 (firing rates target the cap_fr=0.3 scale, so <0.01 is unambiguously dead)
  - silent_rel : peak firing rate < 5% of that network's 95th-percentile unit peak
                 (scale-free; fair across conditions whose overall activity scale differs,
                  e.g. frm-penalized nets are capped near 0.3)
...
def build_task(any_config_path):
    ...
    Args:
        any_config_path: path to any saved <score>_config.yaml (the task config is
            identical across the whole sweep, so one batch serves all nets).
    Returns:
        input_batch: ndarray (n_inputs, T, n_conditions) ready for RNN_numpy.run.
    """
...
def main():
    ...
    # one CDDM batch for all nets (task config is constant across the sweep)
    ...
    # per-net rows, grouped by condition
    ...
    # aggregate + print
    rows = []
    header = f"{'condition':52s} {'nets':>4s} {'N':>5s}  {'dead<0.01':>16s}  {'silent<5%p95':>16s}  {'med_peak':>8s}"
    print(header)
    print("-" * len(header))
