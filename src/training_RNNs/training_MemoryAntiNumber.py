...
P[:, 0] = RNN_valid.W_out
input_as_key = str([0, 0])
points = dsa.fp_data[input_as_key]
all_points = np.vstack([points[type] for type in list(points.keys())])
...
fig_fp = dsa.plot_fixed_points(projection='3D')
if disp:
    plt.show()
if not (datasaver is None): datasaver.save_figure(fig_fp, "slow_points_projection_3D_PCA")
