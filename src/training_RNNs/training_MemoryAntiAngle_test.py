...
from src.Tasks.TaskMemoryAntiAngle import *
...
for tries in range(1):
    ...
    match activation_name:
        ...
        case 'concave_relu': activation = lambda x: torch.pow(torch.maximum(torch.tensor(0.0), x), torch.tensor(0.7))
        case 'convex_relu': activation = lambda x: torch.pow(torch.maximum(torch.tensor(0.0), x), torch.tensor(1.3))
        case 'leaky_relu': activation = lambda x: torch.maximum(torch.tensor(0.0), x) + torch.sign(x) * torch.maximum(torch.tensor(0.0), -0.1 * x)
        case 'cubic_sigmoid': activation = lambda x: ((torch.sign(x) + torch.tensor(1)) / 2) * (x ** 3 / (1 + x ** 3))
